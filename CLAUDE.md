...
npm run prisma:schema:mysql      # generate prisma-mysql/schema.prisma from prisma/schema.prisma
```
...
`DB_TYPE=postgres|mysql` controls which Prisma schema file and adapter are used. `prisma.config.ts` selects `prisma/schema.prisma` or `prisma-mysql/schema.prisma` based on `DB_TYPE`. `PrismaService` creates the corresponding adapter (`PrismaPg` or `PrismaMariaDb`). `prisma-mysql/schema.prisma` is auto-generated from `prisma/schema.prisma` via `npm run prisma:schema:mysql` — only edit `prisma/schema.prisma`. MySQL migrations go in `prisma-mysql/migrations/`.
